...
class PlannerSkeleton:
    ...
    # Default Lot 저점수 슬롯의 final pose 미세 조정값.
    # (slot_idx, occupied < 12) -> (x_offset, y_offset).
    # 원본 코드의 중첩 if/else 조건과 값은 그대로 유지했습니다.
    _DEFAULT_LOW_SCORE_FINAL_OFFSETS: ClassVar[Dict[Tuple[int, bool], Tuple[float, float]]] = {
        (1, True):   (-0.24, -0.28),  # slot 1, 빈 차고
        (1, False):  (-0.24, -0.29),  # slot 1, 차량 존재
        (12, True):  (-0.24, -0.30),  # slot 12, 빈 차고
        # 위 어디에도 안 걸리는 경우엔 default (-0.20, -0.30) 사용
    }
    _DEFAULT_LOW_SCORE_FINAL_DEFAULT_OFFSET: ClassVar[Tuple[float, float]] = (-0.20, -0.30)

    def _get_default_low_score_final_offsets(
        self, slot_idx: int, occupied_count: int
    ) -> Tuple[float, float]:
        key = (slot_idx, occupied_count < 12)
        return self._DEFAULT_LOW_SCORE_FINAL_OFFSETS.get(
            key, self._DEFAULT_LOW_SCORE_FINAL_DEFAULT_OFFSET
        )
    ...
    def _apply_default_low_score_final_tune(
        ...
        final_x_offset, final_y_offset = self._get_default_low_score_final_offsets(
            slot_idx, occupied_count
        )
        tuned[-1] = Waypoint(
    ...
    # Full House waypoint 튜닝 파라미터.
    # (row label) -> (final_x_offset, final_y_offset, nonfinal_speed_scale)
    # row label은 cy가 lower/upper row limit 대비 어느 위치에 있는지로 결정됩니다.
    _FULL_HOUSE_TUNE_BY_ROW: ClassVar[Dict[str, Tuple[float, float, float]]] = {
        "lower":    ( 0.0,   0.10, 1.12),  # cy <= lower_row_limit
        "middle":   (-0.16,  0.18, 1.28),  # lower < cy < upper
        "upper":    (-0.225, 0.35, 1.32),  # cy >= upper_row_limit
    }

    def _classify_full_house_row(self, cy: float) -> str:
        """cy 좌표로 slot의 row를 lower/middle/upper 중 하나로 분류합니다."""

        if self.map_extent is None:
            return "lower"
        _, _, ymin, ymax = self.map_extent
        lower = ymin + (ymax - ymin) * 0.35
        upper = ymin + (ymax - ymin) * 0.65
        if cy >= upper:
            return "upper"
        if cy > lower:
            return "middle"
    ...
    def _apply_full_house_waypoint_tune(
        ...
        final_x_offset, final_y_offset, nonfinal_speed_scale = self._FULL_HOUSE_TUNE_BY_ROW[
            self._classify_full_house_row(cy)
        ]
    ...
    def _build_rear_in_waypoints(self, slot: Rect, open_side: str) -> List[Waypoint]:
        ...
        if open_side == "below":
            ...
            # 슬롯 직전에 수직 정렬 waypoint를 추가해 차량이 틀어지지 않도록 함
            align_y = clamp(slot[2] - 1.8, ymin + 2.5, ymax - 2.5)
            close_left_route = [
                Waypoint(aisle_x, stage_y, "D", radius=1.2, speed=1.4),
                Waypoint(pre_x, stage_y, "D", radius=1.1, speed=1.0),
                Waypoint(cx, stage_y, "D", radius=0.3, speed=0.55),
                Waypoint(cx, align_y, "R", radius=0.25, speed=0.40),
                Waypoint(cx, cy, "R", radius=0.30, speed=0.35, stop_here=True),
            ]
            pull_down_route = [
                ...
                Waypoint(cx, align_y, "R", radius=0.30, speed=0.45),
                Waypoint(cx, cy, "R", radius=0.30, speed=0.35, stop_here=True),
            ]
            if cx <= aisle_x + 10.0:
    ...
    def _steer_toward(
        ...
        # 후면 주차 최종 진입 구간은 lookahead를 줄여 정밀 조향 허용
        if gear == "R" and waypoint.stop_here:
            lookahead = clamp(dist, 0.8, 3.0)
        else:
            lookahead = clamp(dist, 2.0, 6.0)
    ...
    # 속도 / 가속 튜닝 룰
    # ------------------------------------------------------------------
    # 기존 attempt{NN}_* 메서드들은 시뮬레이터 점수를 높이기 위한
    # trial-and-error 튜닝 결과입니다. 동작은 그대로 유지하면서 가독성을
    # 높이기 위해 (1) 순수 predicate 함수들과 (2) 우선순위 기반 룰 테이블로
    # 분리했습니다. 매칭 우선순위는 원본 if/elif 순서와 동일합니다.

    def _current_default_lot_ctx(self) -> Optional[Tuple[int, int]]:
        """현재 타겟이 Default Lot의 저점수 슬롯이면 (slot_idx, occupied)를, 아니면 None을 반환합니다."""

        if self.target_slot is None:
            return None
        if not self._use_default_low_score_final_tune(self.target_slot):
            return None
        slot_idx = self._slot_index(self.target_slot)
        if slot_idx is None:
            return None
        occupied = sum(1 for o in self.occupied_idx if o)
        return slot_idx, occupied

    # --- 순수 predicate: (slot_idx, occupied) -> bool ------------------
    @staticmethod
    def _attempt20_matches(slot_idx: int) -> bool:
        return slot_idx == 1

    @staticmethod
    def _attempt22_matches(slot_idx: int, occupied: int) -> bool:
        return slot_idx == 1 and occupied < 12

    @staticmethod
    def _attempt21_slot12_matches(slot_idx: int) -> bool:
        return slot_idx == 12

    @staticmethod
    def _attempt21_crowded_matches(slot_idx: int, occupied: int) -> bool:
        if occupied < 12:
            return False
        return slot_idx in {1, 9, 12, 14, 15, 16, 17, 18, 19, 24, 25, 26, 27}

    # --- wrapper 메서드 (외부 호출/원본 호환용) -------------------------
    def _use_attempt20_slot1_speed_override(self) -> bool:
        ctx = self._current_default_lot_ctx()
        return ctx is not None and self._attempt20_matches(ctx[0])

    def _use_attempt22_slot1_speed_override(self) -> bool:
        ctx = self._current_default_lot_ctx()
        return ctx is not None and self._attempt22_matches(ctx[0], ctx[1])

    def _use_attempt21_slot12_speed_override(self) -> bool:
        ctx = self._current_default_lot_ctx()
        return ctx is not None and self._attempt21_slot12_matches(ctx[0])

    def _use_attempt21_crowded_speed_override(self) -> bool:
        # 원본은 expected_orientation / map_extent / 슬롯 수 / 라인 수 같은
        # "Default Lot 환경" 검사를 이 메서드에서 수행했습니다. 그대로 유지.
        if self.expected_orientation != "front_in":
            return False
        ...
        if slot_idx is None:
            return False
        occupied = sum(1 for o in self.occupied_idx if o)
        return self._attempt21_crowded_matches(slot_idx, occupied)

    def _use_attempt21_speed2_override(self) -> bool:
        ctx = self._current_default_lot_ctx()
        if ctx is None:
            return False
        slot_idx, occupied = ctx
        return self._attempt21_slot12_matches(slot_idx) or self._attempt21_crowded_matches(
            slot_idx, occupied
        )

    # --- 우선순위 기반 룰 테이블 ---------------------------------------
    # (label, max_loaded, max_empty) 튜플을 위에서부터 순서대로 평가하며
    # 첫 번째로 매칭된 (또는 default) 값이 채택됩니다. 원본 _front_straight_speed_limit의
    # if/elif 순서를 그대로 보존합니다.

    @dataclass(frozen=True)
    class _FrontSpeedRule:
        label: str
        predicate: Optional[str]  # 사용 시점에 _use_attemptXX_* 매서드 이름
        max_loaded: float
        max_empty: float

    _FRONT_SPEED_RULES: ClassVar[Tuple["PlannerSkeleton._FrontSpeedRule", ...]] = (
        _FrontSpeedRule(label="attempt21_speed2", predicate="_use_attempt21_speed2_override",
                        max_loaded=2.5, max_empty=2.65),
        _FrontSpeedRule(label="attempt22_slot1", predicate="_use_attempt22_slot1_speed_override",
                        max_loaded=2.55, max_empty=2.70),
        _FrontSpeedRule(label="attempt20_slot1", predicate="_use_attempt20_slot1_speed_override",
                        max_loaded=2.5, max_empty=2.6),
        _FrontSpeedRule(label="default", predicate=None,
                        max_loaded=2.4, max_empty=2.6),
    )

    def _front_straight_speed_limit(self) -> float:
        occupied = sum(1 for o in self.occupied_idx if o)
        for rule in self._FRONT_SPEED_RULES:
            if rule.predicate is None or getattr(self, rule.predicate)():
                return rule.max_empty if occupied == 0 else rule.max_loaded
        # unreachable
        return 2.4

    # --- 기어 변경 / final pose 감속 거리 룰 ---------------------------
    # 원본 코드의 if 조건을 그대로 룰 테이블로 옮겼습니다.
    def _gear_change_distance(self) -> float:
        return 1.3 if self._use_attempt21_speed2_override() else 1.4

    def _final_brake_distance(self) -> float:
        if (
            self._use_attempt21_slot12_speed_override()
            or self._use_attempt22_slot1_speed_override()
        ):
            return 0.68
        return 0.70

    # --- 가속 게인 룰 --------------------------------------------------
    # (label, predicate, accel_gain, accel_max) 튜플. 첫 매칭 적용.
    @dataclass(frozen=True)
    class _AccelRule:
        label: str
    ...
    _ACCEL_RULES: ClassVar[Tuple["PlannerSkeleton._AccelRule", ...]] = (
        _AccelRule(label="attempt21_speed2", predicate="_use_attempt21_speed2_override",
                   accel_gain=0.46, accel_max=0.92),
        _AccelRule(label="attempt22_slot1", predicate="_use_attempt22_slot1_speed_override",
                   accel_gain=0.46, accel_max=0.92),
        _AccelRule(label="attempt20_slot1", predicate="_use_attempt20_slot1_speed_override",
                   accel_gain=0.48, accel_max=0.92),
        _AccelRule(label="default", predicate=None,
                   accel_gain=0.44, accel_max=0.90),
    )

    def _active_accel_rule(self):
        for rule in self._ACCEL_RULES:
            if rule.predicate is None or getattr(self, rule.predicate)():
                return rule
        return self._ACCEL_RULES[-1]
    ...
    def _speed_command(
        ...
        if idx < len(self.waypoints) - 1 and self.waypoints[idx + 1].gear != waypoint.gear:
            gear_change_dist = (
                1.3 if self._use_attempt21_speed2_override() else 1.4
            )
            if target_dist < gear_change_dist:
                desired_speed = min(desired_speed, 0.75)

        if waypoint.stop_here:
            # 후면 주차일 때는 감속을 더 늦게 시작해 충분한 속도로 슬롯 진입
            if self.expected_orientation == "rear_in":
                desired_speed = min(desired_speed, max(0.18, target_dist * 0.65))
            else:
                desired_speed = min(desired_speed, max(0.12, target_dist * 0.45))
            yaw_error = abs(wrap_angle(yaw - self.final_yaw))
            final_brake_dist = self._final_brake_distance()
            ...
            if (
                ...
                return 0.0, 1.0
            # 후면 주차는 더 가까이 들어온 뒤 정지
            stop_dist = 0.30 if self.expected_orientation == "rear_in" else 0.45
            if target_dist < stop_dist and yaw_error < math.radians(35.0):
                if speed < 0.22:
                    return 0.0, 1.0
                return 0.0, 0.75
        ...
        if speed_error > 0.10:
            rule = self._active_accel_rule()
            accel = 0.18 + rule.accel_gain * speed_error
            return clamp(accel, 0.0, rule.accel_max), 0.0
        if speed_error < -0.15:
